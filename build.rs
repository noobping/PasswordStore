...
fn main() {
    ...
    collect_icon_resources(data_dir, data_dir, &mut icons);
    icons.sort();
...
fn collect_icon_resources(dir: &Path, data_dir: &Path, icons: &mut Vec<String>) {
    for entry in fs::read_dir(dir).expect("Failed to read resource directory") {
        ...
        if path.is_dir() {
            collect_icon_resources(&path, data_dir, icons);
        } else if matches!(
