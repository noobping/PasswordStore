...
fn sync_blocked_by_local_state(status: &StoreGitRepositoryStatus) -> Option<String> {
...
fn configure_store_git_repo_command(cmd: &mut Command, root: &str) {
    cmd.arg("--git-dir").arg(Path::new(root).join(".git"));
}

fn run_store_remote_git_command(
    root: &str,
    context: &str,
    configure: impl FnOnce(&mut Command),
    options: CommandLogOptions,
    ...
    configure(&mut cmd);
    run_command_output(&mut cmd, context, options)
        .map_err(|err| format!("Failed to run remote git command: {err}"))
}

fn run_store_remote_git_work_tree_command(
    root: &str,
    context: &str,
    configure: impl FnOnce(&mut Command),
    options: CommandLogOptions,
    ...
    configure(&mut cmd);
    run_command_output(&mut cmd, context, options)
        .map_err(|err| format!("Failed to run remote work-tree git command: {err}"))
}
...
mod tests {
    use super::{
        add_store_git_remote, configure_store_git_repo_command, has_git_repository,
        list_store_git_remotes, password_store_git_state_summary, remove_store_git_remote,
        rename_store_git_remote, set_store_git_remote_url, store_git_repository_status,
        sync_blocked_by_local_state, sync_store_repository, GitRemote, StoreGitHead,
