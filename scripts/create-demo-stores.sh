...
The demo secret keys use the passphrase: p
EOF
}
...
create_key() {
  ...
  gpg --batch --pinentry-mode loopback --generate-key <<EOF >/dev/null
Key-Type: RSA
Key-Length: 3072
Subkey-Type: RSA
Subkey-Length: 3072
...
EOF
  ...
  gpg --batch --yes --pinentry-mode loopback --passphrase "$KEY_PASSPHRASE" \
    --armor --export-secret-keys "$fingerprint" >"$EXPORTS_DIR/$slug.secret.asc"
...
export KEYCORD_DEMO_PASSPHRASE='$KEY_PASSPHRASE'
EOF
